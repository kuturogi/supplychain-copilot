sap.ui.define(["sap/ui/core/mvc/ControllerExtension", "sap/m/MessageBox", "../util/AIResponseDialog"], function (ControllerExtension, MessageBox, ___util_AIResponseDialog) {
  "use strict";
   ...
   */
  const ObjectPageExt = ControllerExtension.extend("webapp.ext.controller.ObjectPageExt", {
    override: {
      onInit() {
    ...
    // ─── AI Aksiyonları ───────────────────────────────────────────────────────
    onForecastDemand: async function _onForecastDemand() {
    ...
    onCreateOrder: async function _onCreateOrder() {
      const view = this.getView();
      const stockLevelId = view?.getBindingContext()?.getProperty("ID");
      if (stockLevelId === undefined || stockLevelId === null) {
        MessageBox.error("Stok kaydı belirlenemedi.");
    ...
    // ─── Trend Grafiği ────────────────────────────────────────────────────────
    _renderTrendChart: async function _renderTrendChart() {
      ...
      const currentQty = ctx.getProperty("quantity") ?? 0;
      const threshold = ctx.getProperty("criticalThreshold") ?? 0;
      ...
      try {
        ...
        const movements = contexts.map(c => c.getObject()).sort((a, b) => new Date(a.changedAt).getTime() - new Date(b.changedAt).getTime());
        ...
        const facetSection = document.querySelector("[id*='MovementsFacet'], [id*='movementsFacet'], .sapUxAPObjectPageSubSection");
    ...
    /** Mevcut stok ve hareketlerden her nokta için stok seviyesini hesaplar */_computeStockPoints: function _computeStockPoints(currentQty, movements) {
      const points = [];
    ...
    _buildChartHTML: function _buildChartHTML(points, threshold) {
      ...
      const yScale = v => chartH - v / maxQty * chartH;

      // Poliyline nokta dizisi
      const linePoints = points.map((p, i) => `${PAD.left + i * xStep},${PAD.top + yScale(p.qty)}`).join(" ");

      // Alan (fill) için kapalı çokgen
      const areaPoints = `${PAD.left},${PAD.top + chartH} ` + points.map((p, i) => `${PAD.left + i * xStep},${PAD.top + yScale(p.qty)}`).join(" ") + ` ${PAD.left + (points.length - 1) * xStep},${PAD.top + chartH}`;
      ...
      const xLabels = points.filter((_, i) => i % labelStep === 0 || i === points.length - 1).map((p, idx, arr) => {
        const origIdx = idx * labelStep;
        const x = PAD.left + Math.min(origIdx, points.length - 1) * xStep;
        const date = new Date(p.date);
        const label = isNaN(date.getTime()) ? `#${origIdx + 1}` : `${date.getDate()}/${date.getMonth() + 1}`;
...
//# sourceMappingURL=ObjectPageExt-dbg.controller.js.map
