...
let _popover: Popover | null = null;
...
function renderItems(container: VBox, notifications: Notification[], model: ODataModel): void {
    container.destroyItems();

    if (notifications.length === 0) {
        container.addItem(new VBox({
            items: [
                new Icon({ src: "sap-icon://accept", size: "2.5rem" }).addStyleClass("stNotifEmptyIcon"),
                new Text({ text: "Yeni bildirim yok" }).addStyleClass("stNotifEmptyText")
            ]
    ...
    notifications.forEach(n => {
        ...
        const msgText = new Text({ text: n.message, wrapping: true }).addStyleClass("stNotifMsg");
        const metaText = new Text({
            text: `${n.severity}  •  ${formatDate(n.createdAt)}`
        }).addStyleClass("stNotifMeta");

        const leftIcon = new Icon({ src: icon, color }).addStyleClass("stNotifSevIcon");
...
export async function openNotificationPopover(
    opener: HTMLElement | Button,
    model: ODataModel
): Promise<void> {
    ...
    try {
        ...
        notifications = contexts
            .map(c => c.getObject() as Notification)
            .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    } catch (e) {
        console.error("Bildirimler yüklenemedi:", e);
    }

    const itemsContainer = new VBox({ renderType: "Bare" }).addStyleClass("stNotifItemsContainer");
    ...
    _popover = new Popover({
        ...
        showHeader: true,
        content: [scrollArea],
        footer: new Bar({ contentRight: [markAllBtn] }),
        afterClose: () => { _popover?.destroy(); _popover = null; }
    ...
    if (opener instanceof Button) {
        ...
        (_popover as any).openBy(opener);
    }
}

// ─── Shell Header'a bildirim ikonu enjekte et ────────────────────────────────

export function injectNotificationButton(model: ODataModel): void {
    ...
    setTimeout(async () => {
        ...
        try {
            ...
            const res = fnBinding.getBoundContext()?.getObject() as { value?: number };
        ...
        const shellBar =
            document.querySelector(".sapFShellBar") ??
            document.querySelector(".sapMPageHeader") ??
            document.querySelector(".sapUxAPObjectPageHeader");
